...
When you have the official template, put it at `public/certificate-template.png`, then set `useCustomTemplate: true` and adjust `certificateLayout.name` (`x` / `y` are 0–1 positions on the image).
...
When you have the official template, put it at `public/certificate-template.png`, set `useCustomTemplate: true` in `src/config.ts`, and we will align the name line.
